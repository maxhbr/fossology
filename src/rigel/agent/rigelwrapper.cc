...
string scanFileWithRigel(const State& state, const fo::File& file)
{
  FILE* in;
  char buffer[512];
  string command = "rigel " + file.getFileName();
  string result;

  if (!(in = popen(command.c_str(), "r")))
  {
    cout << "could not execute rigel command: " << command << endl;
    bail(1);
  }
...
vector<LicenseMatch> createMatches(vector<string> rigelLicenseNames)
{
  ...
  for (vector<string>::const_iterator it = rigelLicenseNames.begin(); it != rigelLicenseNames.end(); ++it)
  {
    ...
    unsigned percentage = (rigelLicenseName.compare("NONE") == 0 || rigelLicenseName.compare("UNKNOWN") == 0) ? 0 : 100;
